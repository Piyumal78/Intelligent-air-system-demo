...
import { Thermometer, Droplets, Wind, Activity, Waves, AlertTriangle, ArrowDown, Cpu, Sparkles, FileText, Calendar } from "lucide-react"
...
const Dashboard = ({ forceDeviceId }) => {
  ...
  // Derive latest readings dynamically during render directly from Firestore
  ...
  const lastDateObj = parseDate(readings.timestamp)
  const formattedLastDate = lastDateObj ? lastDateObj.toLocaleDateString() : ""
  ...
  const postReadings = {
    co: latest ? Number(latest?.CO_post ?? latest?.co_post ?? 0).toFixed(2) : "0.00",
    lpg: latest ? Number(latest?.LPG_post ?? latest?.lpg_post ?? 0).toFixed(2) : "0.00",
    h2: latest ? Number(latest?.H2_post ?? latest?.h2_post ?? 0).toFixed(2) : "0.00",
    nh3: latest ? Number(latest?.NH3_post ?? latest?.nh3_post ?? 0).toFixed(1) : "0.0",
    temp: latest ? Number(latest?.temperature_post ?? latest?.temp_post ?? readings.temp).toFixed(1) : "0.0",
    hum: latest ? Number(latest?.humidity_post ?? latest?.hum_post ?? readings.hum).toFixed(0) : "0",
  }

  // Filter and smooth chart data directly from Firebase Firestore sensorReadings
  const rawChartData = (sensorReadings || [])
    .filter((r) => r.deviceId === myDevice.id)
    .filter((r) => {
       const dObj = parseDate(r.timestamp)
       if (!dObj) return false
       const diffHours = (now - dObj.getTime()) / (1000 * 60 * 60)
    ...
    .map((r) => {
      const dObj = parseDate(r.timestamp)
      return {
        time: dObj ? dObj.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit", second: "2-digit" }) : "",
        fullDate: dObj ? `${dObj.toLocaleDateString()} ${dObj.toLocaleTimeString()}` : "",
        co: r.CO || 0,
        lpg: r.LPG || 0,
        h2: r.H2 || 0,
        nh3: r.NH3 || 0,
  ...
  const handleGenerateReport = () => {
    ...
    const preRows = chartData.map(d => [d.fullDate || d.time, "Pre-Filtration", d.co, d.lpg, d.h2, d.nh3])
    const postRows = postChartData.map(d => [d.fullDate || d.time, "Post-Filtration", d.co, d.lpg, d.h2, d.nh3])
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          ...
          <div className="mt-2">
            <span className="text-xs font-semibold text-slate-500 bg-slate-100 px-3 py-1.5 rounded-md border border-slate-200 flex items-center gap-1.5 w-fit">
              <Calendar size={14} className="text-slate-400" />
              Last Received: <span className="font-bold text-slate-700">{formattedDateTime}</span>
            </span>
          </div>
        </div>
      ...
      <div className="space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-l-4 border-emerald-500 pl-3 py-0.5">
          <div>
            <h2 className="text-lg font-bold text-slate-700">Live Sensor Feed (Pre-Filtration)</h2>
      ...
      <div className="space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-l-4 border-indigo-500 pl-3 py-0.5">
          <div>
            <h2 className="text-lg font-bold text-slate-700">Filtered Sensor Feed (Post-Purification)</h2>
            ...
            <div className="h-80 w-full ml-[-15px]">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 5 }}>
                  ...
                  <Tooltip labelFormatter={(label, payload) => payload && payload.length > 0 && payload[0].payload?.fullDate ? `Date & Time: ${payload[0].payload.fullDate}` : `Time: ${label}`} contentStyle={{ borderRadius: "12px", border: "none", boxShadow: "0 10px 25px -5px rgb(0 0 0 / 0.1), 0 8px 10px -6px rgb(0 0 0 / 0.1)", padding: "12px" }} itemStyle={{ fontWeight: "bold" }} labelStyle={{ color: "#1e293b", marginBottom: "8px", fontWeight: "bold", fontSize: "12px" }} />
